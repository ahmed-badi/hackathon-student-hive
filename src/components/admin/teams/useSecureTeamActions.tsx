...
interface UseSecureTeamActionsProps {
  teams: Team[];
  unassignedMembers: Registration[];
  onDataChange: () => void;
}

// Input validation for team operations
...
export const useSecureTeamActions = ({ teams, unassignedMembers, onDataChange }: UseSecureTeamActionsProps) => {
  ...
  const createRandomTeams = async () => {
    try {
      await logAction('CREATE_RANDOM_TEAMS', { count: Math.ceil(unassignedMembers.length / 4) });
      ...
      for (let i = 0; i < shuffled.length; i += 4) {
        ...
        if (teamMembers.length >= 2) {
          const teamName = sanitizeTeamName(`Équipe ${teams.length + newTeams.length + 1}`);
          
          if (!validateTeamName(teamName)) {
            throw new Error(`Nom d'équipe invalide: ${teamName}`);
          }
  ...
  const createManualTeam = async (teamName?: string) => {
    try {
      const finalTeamName = sanitizeTeamName(teamName || `Équipe ${teams.length + 1}`);
      
      if (!validateTeamName(finalTeamName)) {
        throw new Error("Nom d'équipe invalide");
      }

      await logAction('CREATE_MANUAL_TEAM', { teamName: finalTeamName });
      
      const { data: teamData, error: teamError } = await supabase
        .from('teams')
        .insert({ name: finalTeamName, track: 'open' })
        .select()
        .single();
  ...
  const deleteTeam = async (teamId: string) => {
    try {
      await logAction('DELETE_TEAM', { teamId });
  ...
  const addMemberToTeam = async (memberId: string, teamId: string) => {
    try {
      await logAction('ADD_TEAM_MEMBER', { memberId, teamId });
  ...
  const removeMemberFromTeam = async (memberId: string, teamId: string) => {
    try {
      await logAction('REMOVE_TEAM_MEMBER', { memberId, teamId });
